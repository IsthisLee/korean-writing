...
- **평소 답변에도 쓸 수 있는 output style 을 넣었습니다.** `/config` 의 Output style 에서 `korean-writing` 을 고르면 글 작성 요청이 아닌 답변에도 규칙이 걸립니다. 기본은 꺼져 있고 `force-for-plugin` 을 쓰지 않아 이미 고른 스타일을 덮지 않습니다. `keep-coding-instructions: true` 라 Claude Code 의 코딩 지침은 그대로 남습니다. 2026-09-11 에 상시 주입을 뺄 때 쓴 기준 넷을 다시 재서 모두 넘겼습니다. 세부 11항목 75/88(기준 67/88), `delay` 기본값 언급 8/8(옛 주입은 0/8), 설계 답변 길이 130%와 127%(하한 85%), 블라인드 문체 3승 1패입니다(`EVALUATION.md` M)
