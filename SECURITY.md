...
설치하면 셸 스크립트 하나가 `.md` 파일을 편집할 때마다 자동으로 실행됩니다. 무엇을 하는지 알고 설치할 수 있도록 아래에 적습니다.
...
| 읽는 것         | 편집한 내용, 편집한 `.md` 파일(줄표를 파일 전체로 셀 때와 걸린 자리의 줄 번호를 찾을 때), 그 파일에서 저장소 루트까지 올라가며 찾은 `.korean-writing.json` |
