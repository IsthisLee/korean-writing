...
> **상태:** 사전 등록 문서입니다(2026-09-15 작성). 2026-09-16 에 파일럿을 마쳤고 결과는 [11. 파일럿 결과](#11-파일럿-결과) 에 있습니다. 2차 파일럿도 마쳤고 결과는 [12. 2차 파일럿 결과](#12-2차-파일럿-결과) 에 있습니다. 본 실행은 하지 않았습니다. 파일럿을 마친 뒤 한 번만 기준을 고칠 수 있고, 고치면 맨 끝 「갱신 이력」에 날짜와 이유를 적습니다. 본 실행 결과를 본 뒤에는 기준을 바꾸지 않습니다.
>
...
아래는 2026-09-15 대화에서 사용자가 정했습니다. 앞의 여덟 줄은 계획 초안 전에, 뒤의 네 줄(개인 블로그, 교정 도구 범위, 「가를 수 없다」 처리, 비용 승인)은 초안을 쓴 뒤에 정했습니다. 「후보 패턴을 만드는 근거」 줄은 2026-09-16 에 더했습니다. 표 끝의 「결과 문서」 앞 일곱 줄 가운데 앞의 여섯 줄은 1차 파일럿 결과를 보고, 「본 실행의 문턱」 줄은 2차 파일럿 결과를 보고 2026-09-16 에 정했습니다.
...
| 블로그 게시일 대조 | Common Crawl 이 2022-11-30 전에 수집한 쪽과 그 본문만 씀 | 파일럿 전에 연구자가 정한 것을 승인. Wayback 을 이틀 모두 쓸 수 없었음([8.7 Wayback 대신 Common Crawl](#87-wayback-대신-common-crawl)) |
| 블로그 표본 틀과 시드 | 개인 블로그는 awesome-devblog `1106089`, 기술 블로그는 awesome-korean-techblog `68fbe20`, 시드 `20260916` | 파일럿 전에 연구자가 정한 것을 승인([4.1 사람 글](#41-사람-글)) |
| 생성 실패와 정리 규칙 | 글 대신 자료를 요청한 응답은 뺌. 제목 줄, 위키 AI 글의 미디어위키 제목 줄, 사용자에게 하는 말인 문단은 사람 글과 AI 글에 같은 규칙으로 뺌 | 파일럿 중에 연구자가 정한 것을 승인([11.2 파일럿 중에 고치거나 정한 것](#112-파일럿-중에-고치거나-정한-것)) |
| 분량 맞추기 | 생성은 한 번만 하고, 분석 때 짝마다 긴 쪽 글을 짧은 쪽 길이에 맞춰 문장 경계에서 자름 | AI 글이 요청 분량보다 짧게 나옴([11.3 사전 등록 기준으로 낸 결과](#113-사전-등록-기준으로-낸-결과)) |
| 번역 위주 블로그 | 표본 틀에서 뺌 | 뺀 곳과 근거는 [4.1 사람 글](#41-사람-글) |
| 다음 단계 | 판정 방식을 고친 뒤 같은 규모의 2차 파일럿 | [4.4 2차 파일럿](#44-2차-파일럿) |
| 본 실행의 문턱 | 계획 5.4 그대로 검증용 사람 글 375편에서 0건이 되도록 정함. 파일럿 30편으로 정한 문턱은 쓰지 않음 | 2차 파일럿에서 AUROC 는 새 표본에서도 유지됐지만 30편으로 정한 문턱은 새 사람 글 6/30편을 넘김([12.5 판단](#125-판단)) |
...
- **번역 위주 블로그 제외(2026-09-16 1차 파일럿 뒤 변경):** 외국 본사 글을 한국어로 옮겨 싣는 블로그는 표본 틀에서 뺍니다. 1차 파일럿의 Amazon Web Services 한국 블로그 글이 번역문으로 보여, 의심되는 곳마다 Common Crawl 수집본 글을 열어 확인했습니다(2026-09-16).
  - **뺀 곳:** Amazon Web Services 한국 블로그는 표본 글 본문에 「아래 게시물은 발표 내용을 요약하여 한국어로 번역한 것입니다」가 있었습니다. Mozilla(`hacks.mozilla.or.kr`)는 표본 글 2편에 「한국어 번역입니다」와 「원저자: … 원문으로 가기」가 있었습니다. Elastic 한국어 블로그는 표본 글 3편과 같은 슬러그의 영어 주소(`elastic.co/blog/…`)가 모두 HTTP 200 으로 열렸습니다.
  - **남긴 곳:** Google 개발자 한국 블로그는 표본 글 4편 가운데 1편에만 「원문은 여기서 확인」과 번역 리뷰 안내가 있었고 나머지 3편에는 번역 표시가 없어, 틀에 남기고 아래 글 단위 규칙으로 거릅니다.
  - **판단하지 않은 곳:** 렘, NGINX STORE, LINE Plus Corporation, 센드버드, 오라클 클라우드, 메가존은 Common Crawl 수집본이 없어 표본으로 뽑히지 않습니다.
  - **글 단위 규칙:** 개인 블로그와 기술 블로그의 모든 글에서, 본문에 「원문은」, 「원문 보기」, 「원문으로 가기」, 「한국어 번역」, 「번역한 것」, 「옮긴이」, 「Translated by」, 「originally published」 같은 명시적 번역 표시가 있으면 뺍니다(`common.TRANSLATION_MARK`).
  - **1차 파일럿:** AWS 글 2편(`tech-03`, `tech-04`)은 문서 점수를 만드는 훈련 자료에서 뺍니다. 11절의 1차 결과 기록은 고치지 않습니다.
...
- **분량:** 목표는 짝이 되는 사람 글의 한글 글자 수를 10 자리에서 반올림한 값입니다. 결과가 목표의 ±20% 를 벗어나면 한 번 다시 쓰게 하고, 두 번째 결과는 벗어나도 쓰되 표시합니다. 이 규칙은 1차 파일럿에만 썼습니다. 다시 써도 대부분 짧았으므로 2차 파일럿부터는 한 번만 쓰고, 분량은 분석 때 짝 맞춤으로 맞춥니다([4.4 2차 파일럿](#44-2차-파일럿)).
...
1차 파일럿 결과([11. 파일럿 결과](#11-파일럿-결과))를 보고, 판정 방식을 고친 뒤 같은 규모로 한 번 더 돌리기로 했습니다(2026-09-16 사용자 결정). 이 절과 5.3 의 「문서 점수」는 2차 파일럿의 AI 글을 만들기 전에 적고 커밋합니다.

- **표본:** 사람 글 30편(위키 10, 개인 블로그 10, 기술 블로그 10)과 짝이 되는 AI 글 30편입니다. 1차 파일럿의 문서, 필자, 회사와 겹치지 않게 합니다. 수집 규칙과 시드는 1차와 같고, 4.1 의 번역 위주 블로그 제외를 더합니다.
- **생성:** 4.2 와 같되 한 번만 씁니다.
- **분석:** 정리(`clean.py`) 뒤 짝마다 긴 쪽 글을 짧은 쪽의 한글 글자 수에 맞춰 문장 경계에서 자르고(`match.py`), 자른 글로 지표를 계산합니다.
- **주 판정:** 5.3 의 「문서 점수」를 1차 파일럿(짝 맞춤, AWS 글 2편 제외)으로 만들고 2차 파일럿으로 확인합니다(`score.py`).
- **가망의 기준:**
  1. 2차 파일럿에서 AI 글 점수와 사람 글 점수의 AUROC 를 구하고, 사람/AI 표시를 1,000번 섞었을 때 관찰값 이상이 나온 비율이 0.05 미만일 것.
  2. 1차 사람 글 점수의 최댓값을 문턱으로 삼았을 때, 2차 사람 글 가운데 문턱을 넘는 글이 0편이고 문턱을 넘는 AI 글이 1편 이상일 것.
- **기준에 따른 다음 단계:** 둘 다 만족하면 본 실행의 설계와 비용을 사용자와 정합니다. 1번만 만족하면 문턱 규칙을 사용자와 다시 정합니다. 1번을 만족하지 못하면 흔적 분석을 계속할지 사용자와 다시 정합니다.
- **보조 결과:** 2차로 만들고 1차로 확인한 점수와, 2차 파일럿의 지표 하나씩 비교(`analyze.py`)도 적습니다. 보조 결과로는 판정하지 않습니다.
...
**문서 점수(2026-09-16 1차 파일럿 뒤 변경).** 1차 파일럿에서 지표 하나로는 가르지 못했으므로, 위 계산 순서의 2단계를 다음처럼 구체화해 주 판정으로 씁니다(`score.py`).

1. 문서마다 값이 하나인 지표 40개를 성격이 같은 20묶음으로 나눕니다(아래 표).
2. 훈련 자료에서 지표마다 사람 글과 AI 글의 평균과 표준편차를 구합니다. 표준편차는 두 묶음을 합친 표준편차의 1/4 보다 작아지지 않게 합니다. 값이 거의 0 인 지표에서 우도비가 튀지 않게 하려는 것입니다.
3. 묶음마다 훈련 자료에서 효과 크기(|Cohen's d|)가 가장 큰 지표 하나를 대표로 고릅니다. 서로 얽힌 지표를 함께 더하지 않으려는 것입니다.
4. 문서 점수는 묶음 대표 지표의 로그 우도비를 정규분포로 계산해 더한 값입니다. 지표 하나의 로그 우도비는 ±3 으로 잘라, 한 지표가 판정을 좌우하지 않게 합니다(3단계의 단일 지표 금지). 값이 없는 지표는 0 으로 둡니다.
5. 문턱은 훈련 자료 사람 글 점수의 최댓값입니다(5.4).

묶음은 1차 파일럿 결과를 본 뒤, 2차 파일럿 자료를 모으기 전에 정했습니다. 그래서 1차로 만든 점수를 2차로 확인합니다.
...
평균·표준편차 하한(1/4), 로그 우도비 자르기(±3), 묶음 나누기는 이 분석이 정한 값이고 출처가 없습니다.
...
12. **점수를 만든 시점:** 문서 점수의 묶음과 계산 방식은 1차 파일럿 결과를 본 뒤에 정했습니다. 1차 자료에 맞춰진 방식일 수 있으므로 1차로 만들고 새 표본인 2차로 확인합니다([4.4 2차 파일럿](#44-2차-파일럿)).
...
2026-09-16 에 [4.4 2차 파일럿](#44-2차-파일럿) 을 돌렸습니다. 표본 목록, 제외·정리 기록, 점수 원본은 `docs/experiments/ai-writing-signals/results/pilot2/` 에 있습니다.

### 12.1 돌린 것

- **위키 글 10편:** 무작위 문서 152개를 봤고, 기준일 전 판이 없던 27개, 동음이의 문서 14개, 한글 800자 미만 101개를 뺐습니다.
- **개인 블로그 글 10편:** 수집본이 없던 필자 30명과 800자 미만 글 15편을 건너뛰었습니다. 1차 파일럿의 필자는 제외했습니다.
- **기술 블로그 글 10편:** 번역 위주 블로그 3곳을 뺀 표본 틀 83곳에서 모았고, 1차 파일럿의 회사는 제외했습니다. 오픈소스컨설팅 2편, 인덴트코퍼레이션 2편, 원티드 제품 팀 1편, 엑셈 2편, OpsNow 2편, VCNC 1편입니다. 수집본이 없던 곳이 20곳, 800자 미만 글이 43편, 글 쪽 표시가 없던 쪽이 27개였습니다.
- **AI 글 30편:** Opus 와 Sonnet 이 15편씩 한 번만 썼습니다. 가짜 도구 호출로 글이 나오지 않은 2편만 12.2 대로 한 번 다시 불렀습니다. 비용은 2.10달러였고(실패한 호출 0.015달러 포함), 한 편에 Opus 는 0.109달러와 1.2분, Sonnet 은 0.030달러와 0.4분이 들었습니다. 목표 분량 대비 비율의 중앙값은 Opus 0.66, Sonnet 0.59 였습니다.

### 12.2 파일럿 중에 고치거나 정한 것

아래는 모두 점수와 지표를 계산하기 전에 했습니다.

1. **제목 따옴표 버그:** 메타 태그 값을 읽는 정규식이 값 안의 작은따옴표에서 멈춰, `tech-06` 의 제목이 「월간기술동향 |」로 잘렸습니다. 값을 감싼 따옴표와 같은 종류까지 읽도록 고치고 AI 글을 쓰기 전에 제목을 바로잡았습니다. 1차·2차의 다른 블로그 제목은 바뀌지 않았습니다.
2. **가짜 도구 호출:** Sonnet 이 쓴 `ai-tech-04` 와 `ai-tech-10` 은 글 없이 「메모리를 먼저 확인하겠습니다」와 `**Tool: skill**`·`**Tool: bash**` 만 남기고 끝났습니다. 도구를 끈 세션에서 호출을 흉내 낸 기술적 실패로 보고, 같은 조건으로 한 번 다시 불렀으며 두 편 모두 글이 나왔습니다. 원 응답과 사유는 `generation_failures.tsv` 에 남겼습니다. 1차 파일럿의 응답 59개에는 같은 흔적이 없었습니다.
3. **정리 규칙 적용:** 승인받은 규칙대로 작성 예고 문단 2개(`ai-tech-04`, `ai-tech-08`)와 위키 AI 글 한 편의 미디어위키 제목 줄 7개를 뺐습니다. `ai-tech-10` 의 첫 줄은 제목의 첫 글자가 가나(「タ다」)로 나와 제목 줄 규칙에 걸리지 않았고 그대로 두었습니다.
4. **번역 표시:** 이 규칙은 개인 블로그 수집을 시작한 뒤에 커밋했으므로, 수집한 사람 글 30편 전체에 같은 식을 적용해 확인했습니다. 걸린 글은 없었습니다.

### 12.3 주 판정: 1차로 만들고 2차로 확인한 문서 점수

훈련 자료는 1차 파일럿의 짝 맞춤 글 54편(AWS 글 2쌍과 생성 실패 1쌍 제외)입니다. 묶음마다 고른 대표 지표는 쉼표 `comma_inclusion`(d=+0.56), 문장부호와 기호 `num_symbol_per_1k`(d=-0.72), 띄어쓰기 `spacing_adherence`(d=+1.09), 어휘와 품사 다양도 `mattr`(d=+1.29), 어휘 밀도 `lexical_density`(d=+0.99), 수식 `adj_ratio`(d=+0.39), 명사화 `nominalization_per_1k`(d=-0.20), 보조 용언 `aux_per_1k`(d=-0.17), 대명사와 지시어 `pronoun_per_1k`(d=+0.18), 조사 `jkg_per_1k`(d=-0.33), 의존명사 `geot_ttaemun_per_1k`(d=-0.08), 문장 길이 `sent_len_cv`(d=-1.02), 절과 의존 구조 `dep_distance`(d=-0.30), 접속 `coord_per_1k`(d=+0.78), 사동·부정·피동 `short_neg_share`(d=-0.42), 양태와 인식 `epistemic_per_1k`(d=-0.22), 담화·완화·정도 `degree_adv_per_1k`(d=+0.19), 번역투 동사 `translationese_verbs_per_1k`(d=-0.84), 높임 `honorific_ep_per_1k`(d=-0.28), 인용 `quote_per_1k`(d=+0.51) 입니다.

| 기준(4.4) | 결과 | 만족 |
| --- | --- | --- |
| 1. AUROC 가 표시를 섞었을 때보다 높은가(섞었을 때 관찰값 이상인 비율 0.05 미만) | AUROC 0.83, 1,000번 섞어 관찰값 이상이 나온 경우 0번 | 예 |
| 2. 1차 사람 글 최댓값(0.63)을 문턱으로 삼았을 때 2차 사람 글 0편, AI 글 1편 이상 | 사람 글 6/30편, AI 글 21/30편이 문턱을 넘음 | 아니요 |

- **장르별:** AUROC 는 위키 0.93, 개인 블로그 0.72, 기술 블로그 0.89 입니다. 문턱을 넘은 사람 글은 위키 2편, 개인 블로그 3편, 기술 블로그 1편이고, 문턱을 넘은 AI 글은 위키 9편, 개인 블로그 5편, 기술 블로그 7편입니다. 개인 블로그에서 가장 덜 갈렸습니다(5.5).
- **모델별:** 사람 글 전체와 비교한 AUROC 는 Opus 0.86, Sonnet 0.80 이고, 문턱을 넘은 AI 글은 Opus 12편, Sonnet 9편입니다.
- **참고:** 2차 사람 글 점수의 최댓값(6.90)보다 높은 AI 글은 4편입니다. 사람 글을 한 편도 잡지 않는 문턱은 AI 글도 대부분 놓칩니다. 이 값은 2차 자료로 문턱을 정한 것이라 판정에 쓰지 않습니다.

### 12.4 보조 결과

- **역방향(2차로 만들고 1차로 확인):** AUROC 0.87, 1,000번 섞어 관찰값 이상이 나온 경우 0번이었습니다. 2차 사람 글 최댓값(5.36)을 문턱으로 삼자 1차 사람 글 1/27편, AI 글 14/27편이 넘었습니다.
- **지표 하나씩(짝 맞춤):** 2차에서 보정 뒤 q < 0.05 인 지표는 0개입니다. 1차(AWS 제외)에서는 7번 의존명사·보조 용언 앞 띄어쓰기 비율, 8번 형태소 MATTR(창 100) 가 q < 0.05 였습니다. 두 파일럿 모두에서 사람 글과 AI 글의 중앙값이 달랐던 지표 가운데 같은 방향이 19개, 반대 방향이 10개였습니다. 한쪽에서라도 q < 0.2 였던 지표는 아래와 같고 모두 같은 방향입니다.

| 지표 | 1차 사람 → AI 중앙값 | 1차 q | 2차 사람 → AI 중앙값 | 2차 q | 방향 |
| --- | --- | --- | --- | --- | --- |
| 7번 의존명사·보조 용언 앞 띄어쓰기 비율 | 0.844 → 0.933 | 0.0209 | 0.847 → 0.952 | 0.106 | 같음 |
| 8번 형태소 MATTR(창 100) | 0.69 → 0.731 | 0.0118 | 0.701 → 0.734 | 0.0592 | 같음 |
| 9번 내용 형태소 비율 | 0.402 → 0.434 | 0.0737 | 0.396 → 0.423 | 0.239 | 같음 |
| 20번 1,000자당 숫자·기호 | 6.34 → 4.78 | 1 | 11.5 → 3.75 | 0.106 | 같음 |
| 25번 문장 길이 변동계수 | 0.56 → 0.472 | 0.094 | 0.544 → 0.44 | 0.0592 | 같음 |
| 28번 1,000형태소당 접속 조사와 '및' | 5.04 → 10.2 | 0.17 | 4.03 → 8.59 | 0.282 | 같음 |
| 39번 1,000형태소당 대하다·의하다·만들다·가지다 | 3.94 → 2.04 | 0.159 | 3.28 → 1.79 | 1 | 같음 |

### 12.5 판단

1. **여러 지표를 합친 점수는 새 표본에서도 사람 글과 AI 글을 갈랐습니다.** 1차로 만든 점수가 2차에서 AUROC 0.83, 반대 방향에서 0.87 이었고, 두 방향 모두 표시를 섞었을 때보다 높았습니다.
2. **「사람 글 0건」 문턱은 30편으로 정할 수 없었습니다.** 1차 사람 글 최댓값을 문턱으로 삼자 새 사람 글의 6/30편이 넘었습니다. 4.4 에 따라 문턱 규칙을 사용자와 다시 정했고, 사용자는 계획 5.4 대로 본 실행의 검증용 사람 글 375편에서 문턱을 정하기로 했습니다(2026-09-16, 3절).
3. **본 실행 비용 견적(한 번만 쓰는 조건):** AI 글 1,500편에 약 104달러, 한 번에 한 편씩 돌리면 약 19시간입니다. 가짜 도구 호출을 다시 부르는 비용은 2차에서 0.015달러였습니다. 플러그인 측정과 판정 비용은 들어 있지 않습니다.
...
- 2026-09-16(1차 파일럿 뒤 기준 수정, 한 번 허용된 수정): 사용자가 11절 결과를 보고 네 가지를 정했습니다. 판정 방식을 고친 뒤 같은 규모의 2차 파일럿을 돌리고(4.4), 분량은 생성 때가 아니라 분석 때 짝 맞춤으로 맞추고(4.2, 4.4), 번역 위주 블로그를 표본 틀에서 빼고(4.1), 파일럿 전·중에 연구자가 정한 결정을 모두 승인했습니다(3절). 이에 따라 여러 지표를 합친 문서 점수를 5.3 에 정의하고 2차 파일럿의 가망 기준을 4.4 에 적었습니다. 번역 블로그를 가린 확인과 글 단위 번역 표시 규칙은 4.1 에 있습니다. 2차 파일럿의 사람 글 수집(위키, 개인 블로그)은 이 수정을 커밋하기 전에 시작했고, 개인 블로그 글에는 글 단위 번역 표시 규칙을 수집이 끝난 뒤 같은 식으로 적용합니다. 기술 블로그 수집, AI 글 생성, 지표 계산은 커밋한 뒤에 돌립니다.
- 2026-09-16(2차 파일럿 결과): 2차 파일럿을 돌리고 12절에 결과를 적었습니다. 점수를 계산하기 전에 제목 따옴표 버그를 고쳤고, 가짜 도구 호출 2편을 같은 조건으로 한 번 다시 불렀습니다(12.2). 가망 기준 가운데 1번(AUROC)은 만족하고 2번(사람 글 0편 문턱)은 만족하지 못해, 4.4 에 따라 문턱 규칙을 사용자와 다시 정합니다.
- 2026-09-16(2차 파일럿 뒤 결정): 사용자가 12절 결과를 보고, 본 실행의 문턱을 계획 5.4 그대로 검증용 사람 글 375편에서 정하기로 했습니다. 파일럿 30편으로 정한 문턱은 쓰지 않습니다. 기준을 바꾼 것이 아니라 원래 계획의 문턱 규칙을 유지한 결정이며 3절에 적었습니다.
