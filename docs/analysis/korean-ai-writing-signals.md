...
- **Q5.** 검사 훅(K 규칙)에 남길 규칙은 무엇인가. 2026-09-16 에 더했습니다([5.7 검사 훅 규칙 개편](#57-검사-훅-규칙-개편)).

## 3. 확정한 결정

아래는 2026-09-15 대화에서 사용자가 정했습니다. 앞의 여덟 줄은 계획 초안 전에, 뒤의 네 줄(개인 블로그, 교정 도구 범위, 「가를 수 없다」 처리, 비용 승인)은 초안을 쓴 뒤에 정했습니다. 「후보 패턴을 만드는 근거」 줄은 2026-09-16 에 더했습니다. 표 끝의 「결과 문서」 앞 열 줄 가운데 앞의 여섯 줄은 1차 파일럿 결과를 보고, 「본 실행의 문턱」 줄은 2차 파일럿 결과를 보고, 「본 실행 표본 수」 줄은 기술 블로그 수용량을 확인한 뒤 2026-09-16 에 정했습니다. 「검사기의 위치」와 「검사 훅 규칙」 줄은 본 실행 사람 글을 모으는 중이고 AI 글을 만들기 전인 2026-09-16 에 정했습니다.
...
| 본 실행의 문턱 | 계획 5.4 그대로 검증용 사람 글에서 0건이 되도록 정함(정할 때는 375편, 아래 줄에서 300편으로 바뀜). 파일럿 30편으로 정한 문턱은 쓰지 않음 | 2차 파일럿에서 AUROC 는 새 표본에서도 유지됐지만 30편으로 정한 문턱은 새 사람 글 6/30편을 넘김([12.5 판단](#125-판단)) |
| 본 실행 표본 수 | 기술 블로그 글 150편(회사마다 15편까지). 사람 글은 모두 600편(선별용·검증용 300편씩), AI 글은 1,200편 | 수집본이 있는 회사가 35곳뿐이고 글이 몇 곳에 몰려, 300편은 회사 편중 없이 모을 수 없음([8.9 기술 블로그 수용량](#89-기술-블로그-수용량)) |
| 검사기의 위치 | 분석 도구(5.1)와 문서 점수는 저장소(`docs/experiments/ai-writing-signals/`)에만 둠. 설치본(`plugin/`)에는 넣지 않음 | 사용자 결정 |
| 검사 훅 규칙 | 본 실행 표본에서 사람 글을 한 편도 잡지 않고 Claude 글을 잡은 규칙으로만 다시 구성. 지금 규칙(K1~K4, K6~K8)도 같은 기준으로 판정해 사람 글을 잡으면 뺌. 문턱은 규칙마다 최소 횟수 | 후보는 지금 규칙과 선별용 표본에서 뽑은 표현. 세부는 [5.7 검사 훅 규칙 개편](#57-검사-훅-규칙-개편) |
...
- **수:** 600편입니다(2026-09-16 에 750편에서 줄임, 3절). 패턴을 고르는 선별용 300편과 문턱을 확인하는 검증용 300편으로 나눕니다. 검증용이 299편 이상이어야 하는 이유는 [5.4 문턱과 표본 수](#54-문턱과-표본-수) 에 있습니다.
- **장르:** 위키 글 300편, 기술 블로그 글 150편, 개인 블로그 글 150편이고, 장르마다 선별용과 검증용에 반씩 넣습니다.
...
- **짝:** 사람 글 한 편마다 같은 제목, 같은 장르, 비슷한 분량(±20%)으로 두 모델이 한 편씩 씁니다. 사람 글 600편이면 모두 1,200편입니다.
...
- α = 1%, δ = 5% 이면 299편이 필요합니다([8.6 Neyman-Pearson 표본 수 계산](#86-neyman-pearson-표본-수-계산)). 검증용 300편은 이 조건을 넘습니다.
- 150편이면 같은 δ 에서 보장되는 α 가 약 1.98%로 올라갑니다([8.6 Neyman-Pearson 표본 수 계산](#86-neyman-pearson-표본-수-계산)).

0건이 나왔을 때의 상한은 「3의 규칙」으로도 확인합니다[\[S6\]](#s6). n 편에서 0건이면 95% 신뢰 상한은 약 3/n 이므로, 300편이면 1% 입니다. 이 두 계산이 검증용 사람 글을 300편 가까이 이상 모으는 이유입니다. 개인 블로그를 더하면서 검증용은 375편이 됐다가, 2026-09-16 에 기술 블로그를 150편으로 줄이면서 300편이 됐습니다. 300편에서 0건이면 같은 δ 에서 보장되는 α 는 약 0.99% 입니다(`python3 -c "print(1-0.05**(1/300))"`).
...
2026-09-16 에 사용자가 정했습니다. 본 실행 사람 글을 모으는 중이었고, AI 글은 만들기 전이며, 표본에서 표현 빈도는 아직 세지 않았습니다. 3절의 기존 기준은 바꾸지 않고 이 판정을 더합니다.

검사 훅의 K 규칙을 본 실행 표본에서 **사람 글을 한 편도 잡지 않고 Claude 글을 잡은 규칙으로만** 다시 구성합니다. 지금 규칙도 같은 기준을 거치며, 사람 글을 한 편이라도 잡으면 뺍니다.

- **조건:** 설치본의 훅은 형태소 분석기 없이 python3 표준 라이브러리의 정규식으로만 돕니다. 그래서 후보는 정규식으로 옮길 수 있어야 합니다. 분석 도구(5.1)는 저장소에만 두고 설치본에는 넣지 않습니다(3절).
- **판정에 쓰는 글:** 정리(`clean.py`)와 짝 맞춤(`match.py`)을 거친 사람 글 600편과 Claude 글 1,200편입니다. 규칙은 `plugin/scripts/check.sh` 처럼 글 한 편 전체에 적용합니다.
- **후보 1, 지금 규칙:** `plugin/hooks-handlers/posttooluse.sh` 에 있는 K1, K2, K3, K4, K6, K7, K8 의 정규식을 그대로 씁니다. K8 은 항목마다 횟수를 따로 세므로 `가지고 있`, `되어지`, `에 의해` 를 각각 후보로 봅니다.
- **후보 2, 표본에서 뽑은 표현:** 선별용 표본만 씁니다. 글을 공백으로 나눈 어절에서 앞뒤 문장부호를 떼고, 이어진 어절 1~3개(1~3-gram)를 셉니다. 5.3 의 1단계와 같은 방식(분산을 보정한 로그 오즈 z, α0 = 1,000)으로 Claude 글 쪽 z 를 구합니다. z ≥ 3 이면서 선별용 Claude 글 5편 이상에 나온 표현을 z 가 큰 순서로 200개까지 후보로 삼습니다. 후보는 문자열 그대로 정규식으로 옮기고 어절 경계(앞뒤가 공백, 문장부호, 글의 처음이나 끝)만 더하며, 사람이 손으로 일반화하지 않습니다.
- **문턱:** 후보마다 선별용 사람 글 300편에서 한 편에 가장 많이 나온 횟수를 구하고, 거기에 1을 더한 값을 문턱으로 삼습니다. 글 한 편에서 후보가 문턱 이상 나오면 그 글을 「잡았다」고 봅니다. 지금 규칙의 횟수(예: K2 3회)는 쓰지 않고 이 방법으로 다시 정합니다.
- **채택 기준:** 두 조건을 모두 만족한 후보만 K 규칙으로 채택합니다. 3절 「패턴 채택 기준」의 「그 패턴에만 걸리는 AI 글」 조건은 이 판정에 쓰지 않습니다.
  1. 사람 글 600편(선별용과 검증용) 가운데 잡힌 글이 0편일 것.
  2. 검증용 Claude 글 600편 가운데 잡힌 글이 1편 이상일 것.
- **번호:** 채택된 지금 규칙은 코드를 그대로 두고, 새로 채택한 규칙에는 K11 부터 번호를 붙입니다. 이미 뺀 번호(K5, K9, K10)와 이번에 빠지는 번호는 다시 쓰지 않습니다.
- **알림 문구:** 새 규칙의 알림은 `plugin/skills/humanize-korean/references/rewriting-playbook.md` 에 해당 처방이 있으면 그 처방을 따르고, 없으면 같은 뜻의 다른 표현으로 바꾸되 사실과 문장 성분을 빼지 않게 안내합니다. 알림을 바꾸면 `docs/experiments/detail-retention/run.sh` 로 세부 보존을 재고, `tests/test_posttooluse.py` 에 회귀 테스트를 넣고, EVALUATION.md 에 기록합니다.
- **보고:** 후보마다 문턱, 선별용·검증용 사람 글에서 잡힌 편수, 검증용 Claude 글에서 잡힌 편수를 장르별·모델별로 적습니다. 채택한 규칙을 모두 합쳤을 때 사람 글과 Claude 글에서 잡힌 편수도 적습니다.
- **정한 값:** 1~3-gram, z ≥ 3, Claude 글 5편 이상, 후보 200개는 이 분석이 정한 값이고 출처가 없습니다.

한계는 셋입니다.

1. **표본 밖의 오탐률은 보장하지 않습니다.** 검증용 사람 글은 문턱을 정하는 데 쓰지 않지만 채택 여부를 가르는 데는 씁니다. 후보가 많을수록 우연히 0편이 나온 규칙이 섞이므로, 5.4 의 「오탐률 1% 이하를 95% 확신」은 채택한 규칙에 그대로 적용되지 않습니다.
2. **훅은 편집한 부분만 봅니다.** 문턱은 글 한 편 전체에서 정한 횟수라서, 짧게 고친 편집에서는 같은 표현이 문턱에 덜 닿습니다.
3. **표현 후보는 어절 단위입니다.** 여러 동사에 붙는 같은 어미처럼 어절의 일부만 겹치는 표현은 따로 잡지 못합니다.
...
플러그인마다 격리해 설치하고 실행하는 스크립트가 필요합니다. 이 저장소의 이전 측정에서 `--plugin-dir` 로 올린 output style 은 `--setting-sources` 를 세 가지로 바꿔도 적용되지 않았습니다(커밋 `92b1e47` 까지의 EVALUATION.md M절 「이 측정이 못 보는 것」 5번)[\[R1\]](#r1). 그래서 스타일이 실제로 켜졌는지를 먼저 확인하는 절차를 둡니다.
...
4. **분석:** 등급과 문턱을 정하고 검증용 표본으로 확인합니다. 검사 훅 규칙도 [5.7 검사 훅 규칙 개편](#57-검사-훅-규칙-개편) 대로 다시 정합니다.
...
본 실행(AI 글 생성 1,200회, 플러그인 측정, 판정)은 파일럿 뒤에 승인받습니다. 파일럿에서 잰 호출당 비용과 시간으로 견적을 내어 함께 보고합니다.
...
2026-09-16 에 번역 위주 블로그를 뺀 표본 틀 83곳마다 Common Crawl 색인(CC-MAIN-2022-40, 2022-33, 2022-21)에서 기준일 전에 수집된 글 주소 수를 셌습니다(`out/tech-capacity.tsv`). 수집본 글이 있는 곳은 35곳이었고, 10개 이상인 곳은 22곳이었습니다. 가장 많은 곳은 LG CNS 1,170개, Google 855개, 카카오 201개였습니다. 파일럿에서 수집본 글 주소 가운데 기준을 통과한 비율은 12~20%였으므로, 회사마다 15편 상한으로는 약 150편, 상한이 없으면 약 440편이 모이고 그 절반 이상이 두 곳의 글이 된다고 어림했습니다.

같은 날 2차 파일럿의 호출당 비용(Opus 0.109달러, Sonnet 0.030달러)과 시간(1.2분, 0.4분)으로 계산하면, 사람 글 600편에 두 모델이 한 편씩 쓰는 AI 글 1,200편은 약 84달러이고 한 번에 한 편씩 돌리면 약 16시간입니다.
...
- <a id="r1"></a>**[R1]** `EVALUATION.md` O절(검사 훅 처방과 O3 무효 판단), M절 「이 측정이 못 보는 것」 5번(`--plugin-dir` 로 output style 이 적용되지 않음). M절은 2026-09-16 에 지웠고 커밋 `92b1e47` 판에 있습니다. 커밋 `b2e2089`.
...
2. **「사람 글 0건」 문턱은 30편으로 정할 수 없었습니다.** 1차 사람 글 최댓값을 문턱으로 삼자 새 사람 글의 6/30편이 넘었습니다. 4.4 에 따라 문턱 규칙을 사용자와 다시 정했고, 사용자는 계획 5.4 대로 본 실행의 검증용 사람 글에서 문턱을 정하기로 했습니다(2026-09-16, 3절). 검증용은 그 뒤 기술 블로그를 줄이며 300편이 됐습니다.
...
- 2026-09-16(본 실행 표본 수): 기술 블로그 수용량(8.9)을 보고 사용자가 본 실행의 기술 블로그 글을 150편, 회사마다 15편까지로 정했습니다. 사람 글은 600편(선별용·검증용 300편씩), AI 글은 1,200편이 됩니다. 검증용 300편은 5.4 의 299편 조건을 넘습니다. 본 실행 수집을 시작한 뒤, AI 글 생성 전에 정한 변경입니다.
- 2026-09-16(검사기 위치와 검사 훅 규칙): 사용자가 두 가지를 정했습니다. 분석 도구와 문서 점수는 저장소에만 두고 설치본에는 넣지 않습니다. 검사 훅의 K 규칙은 본 실행 표본에서 사람 글을 한 편도 잡지 않고 Claude 글을 잡은 규칙으로만 다시 구성하고, 지금 규칙도 같은 기준을 거칩니다(Q5, 5.7). 후보 범위(지금 규칙과 선별용 표본에서 뽑은 표현)와 문턱 방식(규칙마다 최소 횟수)은 사용자가 골랐고, 표현을 뽑는 세부값은 연구자가 정했습니다. 본 실행 사람 글 수집 중, AI 글 생성과 표현 빈도 계산 전의 추가이며 기존 기준은 바꾸지 않았습니다.
