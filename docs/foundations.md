...
이 플러그인이 무엇을 읽고 무엇을 기준으로 한국어를 판정하는지 한자리에 모은 문서입니다. 근거가 여덟 곳에 흩어져 있어 처음 보는 사람이 어디부터 열어야 할지 알기 어려웠습니다.

**이 문서는 지도입니다.** 본문을 옮겨 적지 않고 어디에 무엇이 있는지만 적습니다. 같은 사실을 두 곳에 두면 한쪽만 고쳐질 때 틀린 쪽이 남습니다. 원본이 바뀌면 여기서는 한 줄 설명만 고칩니다.
...
| 측정 기록 | [EVALUATION.md](../EVALUATION.md) A~M절 | 훅 정확도, 스킬 트리거, 비용, 주입이 일을 방해하는가, 블라인드 판정의 신뢰도, output style의 세부 보존 |
