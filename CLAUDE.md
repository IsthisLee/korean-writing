...
**korean-writing 작성 스킬은 2026-09-15 에 뺐습니다.** 평소 답변과 처음 쓰는 글의 문체는 사용자가 고른 output style 이 맡습니다. 2026-09-14 에 쓰기 전 확인 훅(`pretooluse-skill.sh`)과 이 저장소의 output style 을 뺐고, 이튿날 `/korean-writing` 으로 부르던 스킬도 뺐습니다. 스킬 본문은 C·J·L 절 측정을 재현할 수 있도록 `docs/experiments/writing-skill.md` 로 옮겼고, 스킬로 다시 잡히지 않게 파일 이름을 `SKILL.md` 로 두지 않았습니다. 작성 스킬을 되살리려면 EVALUATION.md J3 의 세부 손실 측정을 다시 통과해야 합니다.
